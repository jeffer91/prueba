const { runFfprobeJson } = require('../../services/ffmpegService');
...
async function readMediaMetadata(videoPath) {
  if (!videoPath) {
    throw new Error('No se recibió la ruta del video.');
  }
  ...
  if (!videoStream) {
    throw new Error('El archivo no contiene un stream de video válido.');
  }

  return {
    durationSeconds: toNumberOrNull(videoStream.duration) || toNumberOrNull(probe.format && probe.format.duration),
